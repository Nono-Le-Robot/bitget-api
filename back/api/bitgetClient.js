...
async function bitgetRequest(method, endpoint, params = {}) {
  ...
  const requestPath = `${endpoint}${method === 'GET' ? `?${new URLSearchParams(params).toString()}` : ''}`;
...
export async function openOrder(orderData) {
  ...
  return await bitgetRequest('POST', '/api/v2/mix/order/batch-place-order', params);
}

// Exemple de fonctions pour obtenir des données
export async function getFuturesPositions(productType, marginCoin) {
  return await bitgetRequest('GET', '/api/v2/mix/position/all-position', { productType, marginCoin });
}

export async function getFuturesAccount(productType) {
  return await bitgetRequest('GET', '/api/v2/mix/account/accounts', { productType });
}
